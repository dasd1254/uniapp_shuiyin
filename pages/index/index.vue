<template>
	<view class="container">
		...
		<view v-else-if="step === 'paint'" class="paint-zone">
			<view class="canvas-wrapper" id="finalCanvasWrapper"
				:style="{ width: imgWidth + 'px', height: imgHeight + 'px' }" style="border: 1px solid red;">
				<image :src="originalImage" class="bg-img"
					:style="{ width: imgWidth + 'px', height: imgHeight + 'px' }" />

				<canvas v-if="canvasReady" canvas-id="maskCanvas" id="maskCanvas" class="mask-canvas"
...
<script setup>
	...
	// 请确认这个 IP 在手机上能访问（手机和电脑连同一个 WiFi，或者部署到了公网）
	...
	const calibrateSize = () => {
		...
		if (wrapper) {
			...
			if (rect.width > 0 && rect.height > 0) {
				...
				console.log(
					`📱 手机端校准: 物理${imgWidth.value}x${imgHeight.value} / 视觉${rect.width.toFixed(0)}x${rect.height.toFixed(0)}`
					);
	...
	const getPoint = (e) => {
		...
		// 1. 优先检测触摸事件 (Touch) - 手机端靠这个
		if (e.touches && e.touches.length > 0) {
			clientX = e.touches[0].clientX;
			clientY = e.touches[0].clientY;
		}
		// 2. 其次检测触摸结束事件 (ChangedTouch)
		else if (e.changedTouches && e.changedTouches.length > 0) {
			clientX = e.changedTouches[0].clientX;
			clientY = e.changedTouches[0].clientY;
		}
		// 3. 最后检测鼠标事件 (Mouse) - PC端靠这个
		else {
			clientX = e.clientX;
			clientY = e.clientY;
		}

		// 计算坐标：(触点绝对位置 - 画布左上角位置) * 缩放比例
	...
	const handleStart = (e) => {
		// 【关键】阻止默认行为，防止手机端滚动页面
		if (e.cancelable && !isSliderDragging.value) {
			e.preventDefault();
		}
		...
		ctx.setLineWidth(lineWidth.value * scaleX.value);
		ctx.setStrokeStyle('rgba(255, 255, 255, 0.8)');
		ctx.setLineCap('round');
		ctx.setLineJoin('round');
	...
	const handleEnd = () => {
		if (isDrawing.value) {
			...
			if (drawHistory.value) drawHistory.value.push({
				width: lineWidth.value * scaleX.value,
				points: [...currentPath]
			});
			currentPath = [];
