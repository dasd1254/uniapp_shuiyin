  ...
  uni.chooseImage({
    ...
    success: (res) => {
      ...
      uni.getImageInfo({
        src: filePath,
        success: (image) => {
          // 【修复2】这里必须调用！先算个大概尺寸，否则页面是 0px 啥也看不见
          calculateInitialSize(image.width, image.height);
          step.value = 'paint';
          
          // 2. 等待 DOM 渲染后，再进行精准校准
          nextTick(() => {
...
const calculateInitialSize = (w, h) => {
  ...
  // 【修复3】移除 max-height 限制，或者设得很大，防止图片被截断导致涂抹不到底部
  // 现在的逻辑是：让图片自适应宽度，高度随之变化，不强行限制高度
...
const calibrateSize = () => {
  ...
  if (wrapper) {
    ...
    if (rect.width > 0 && rect.height > 0) {
      // 强制同步：把 JS 里的尺寸更新为屏幕上真实的尺寸
      imgWidth.value = rect.width;
      imgHeight.value = rect.height;
      
      console.log(`📏 最终校准: ${imgWidth.value} x ${imgHeight.value}`);
...
const handleEnd = () => {
  if (isDrawing.value) {
    ...
    if (drawHistory.value) drawHistory.value.push({ width: lineWidth.value, points: [...currentPath] });
    currentPath = [];
